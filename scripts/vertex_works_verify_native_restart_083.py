...
def run(label, args):
    print(f"RUN {label}: {' '.join(map(str, args))}")
    ...
    if cp.stderr:
        print(cp.stderr)
    if cp.returncode != 0:
        raise SystemExit(f"{label} FAIL exit={cp.returncode}")
...
# Correct contract:
# - JS arms through setState("armed", "CONFIRM")
# - CSS renders button[data-state="armed"]
# The previous verifier incorrectly required a literal HTML data-state="armed"
# inside JS even though the runtime state is set programmatically.
two_click_guard = (
    "const ARM_MS = 2400" in js
    and 'setState("armed", "CONFIRM")' in js
    and "Date.now() <= armedUntil" in js
    and 'button[data-state="armed"]' in css
)
...
checks = [
    ...
    emit("ARM_WINDOW_2400MS", "const ARM_MS = 2400" in js),
    emit("ARM_STATE_PROGRAMMATIC", 'setState("armed", "CONFIRM")' in js),
    emit("SECOND_CLICK_GUARD", "Date.now() <= armedUntil" in js),
    emit("ARMED_STYLE_PRESENT", 'button[data-state="armed"]' in css),
    emit("TWO_CLICK_GUARD", two_click_guard),
    ...
    emit(
        "HANDLER_REGISTERED",
        bool(re.search(r"generate_handler!\[[\s\S]*?\brestart_vertex_works\s*,", main)),
    ),
...
if not all(checks):
    raise SystemExit("VERTEX_WORKS_NATIVE_RESTART_083_VERIFY FAIL")

# Reconfirm native source still compiles/tests without rebuilding another release.
run("CARGO_FMT_CHECK", ["cargo", "fmt", "--manifest-path", str(CARGO), "--", "--check"])
run("CARGO_TEST", ["cargo", "test", "--manifest-path", str(CARGO)])

print("RESTART_IMPLEMENTATION_SOURCE=000082")
print("VERIFICATION_HOTFIX=000083")
print("NO_PRODUCT_SOURCE_MUTATION=TRUE")
print("CURRENT_RELEASE=" + str(release))
print("VERTEX_WORKS_NATIVE_RESTART_083_VERIFY PASS")
