...
safe_print("VERTEX WORKS RAY / BLUEPRINT CAPABILITY PROBE 000085")
...
# UI contract presence does NOT prove runtime behavior.
relation_ids = [
...
for ident in relation_ids:
    safe_print(f"UI_{ident}={'PRESENT' if f'id=\"{ident}\"' in index else 'ABSENT'}")
safe_print("RELATION_UI_CONTRACT=" + ("PRESENT" if all(f'id=\"{i}\"' in index for i in relation_ids) else "INCOMPLETE"))
safe_print("NOTE=UI_CONTRACT_IS_NOT_RUNTIME_PROOF")

# Token-level implementation evidence.
combined_js = xjs + "\n" + app
tokens = {
...
# Theme audit. Blue/cyan may be legitimate semantic relation colors; count only, do not auto-fail.
theme_text = xcss + "\n" + orange + "\n" + factory_css
...
# Exact source context for next patch.
...
safe_print("\nVERTEX_WORKS_RAY_BLUEPRINT_PROBE_085 PASS")
